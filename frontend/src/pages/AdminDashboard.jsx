...
import {
  Users, FileText, BarChart3, ShieldCheck, Trash2,
...
export default function AdminDashboard() {
  ...
  useEffect(() => { if (tab === "petitions") fetchPetitions(); }, [tab, petitionFilter]);
  useEffect(() => { if (tab === "polls") fetchPolls(); }, [tab]);
  ...
  const fetchUsers = async () => {
    setLoading(true);
    try {
      const res = await axios.get(`${API}/users`, { headers, params: { search, role: roleFilter } });
      setUsers(res.data);
  ...
  const fetchPetitions = async () => {
    setLoading(true);
    try {
      const res = await axios.get(`${API}/petitions`, { headers, params: { status: petitionFilter } });
  ...
  const approvePetition = async (id) => {
    ...
    fetchPetitions(); fetchStats();
  };

  const disapprovePetition = async (id) => {
    setActionLoading(p => ({ ...p, [id + "_disapprove"]: true }));
    await axios.put(`${API}/petitions/${id}/reject`, { reason: "Disapproved by admin" }, { headers });
    ...
    <div className="space-y-8">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-slate-500 text-sm">Manage the entire platform from here.</p>
        </div>
      </div>
          ...
          return (
            <button
              ...
              className={`flex items-center gap-2 px-5 py-2 rounded-xl text-xs font-black transition-all capitalize tracking-widest ${
                tab === t.id ? "bg-white text-purple-600 shadow-md" : "text-slate-500 hover:text-slate-800"
              }`}
            >
      ...
      <AnimatePresence mode="wait">
          ...
          <motion.div key="overview" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="space-y-8">
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
              {[
                ...
                { label: "Total Petitions", value: stats.totalPetitions, icon: FileText, color: "bg-blue-500" },
                { label: "Pending Review", value: stats.pendingPetitions, icon: AlertCircle, color: "bg-amber-500" },
                { label: "Total Polls", value: stats.totalPolls, icon: BarChart3, color: "bg-green-500" },
              ].map((s, i) => (
                <motion.div
                  key={s.label}
            ...
            <div className="card p-6">
              <h3 className="text-sm font-black text-slate-900 uppercase tracking-widest mb-6">User Role Breakdown</h3>
              <div className="flex flex-wrap gap-4">
                {stats.roleBreakdown.map((r) => (
                  <div key={r._id} className="flex items-center gap-3 bg-slate-50 rounded-2xl px-5 py-4 border border-slate-100">
                    <span className={roleBadge(r._id)}>{r._id}</span>
                    <span className="text-2xl font-black text-slate-900">{r.count}</span>
                  </div>
                ))}
              </div>
            </div>
        ...
        {/* ── PETITIONS ── */}
        {tab === "petitions" && (
          <motion.div key="petitions" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="space-y-6">
            <div className="flex flex-wrap gap-3 items-center">
              ...
              <button onClick={fetchPetitions} className="p-2.5 bg-slate-100 rounded-xl hover:bg-slate-200 transition-colors">
                <RefreshCw className="w-4 h-4 text-slate-500" />
              </button>
            </div>

            {loading ? (
              <div className="flex justify-center py-16"><div className="w-10 h-10 border-4 border-purple-200 border-t-purple-600 rounded-full animate-spin" /></div>
            ) : (
              <div className="space-y-4">
                {petitions.map((p) => (
                  <motion.div key={p._id} layout initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="card p-6">
                    <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
                      <div className="flex-1 space-y-2">
                        ...
                        <div className="flex flex-wrap gap-4 text-xs text-slate-400 font-bold pt-1">
                          ...
                          <span>Signatures: {p.signatures?.length || 0} / {p.goal}</span>
                      ...
                      <div className="flex flex-wrap gap-2 shrink-0">
                        <motion.button
                          ...
                          disabled={p.status === "active" || actionLoading[p._id + "_approve"]}
                        ...
                        <motion.button
                          ...
                          disabled={p.status === "closed" || actionLoading[p._id + "_disapprove"]}
                  ...
                  <tbody className="divide-y divide-slate-50">
                    {users.map((u) => (
                      <tr key={u._id} className="hover:bg-slate-50 transition-colors">
                        ...
                        <td className="px-6 py-4 text-slate-500">{u.location || "—"}</td>
        ...
        {/* ── POLLS ── */}
        {tab === "polls" && (
          <motion.div key="polls" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="space-y-4">
            {loading ? (
              <div className="flex justify-center py-16"><div className="w-10 h-10 border-4 border-purple-200 border-t-purple-600 rounded-full animate-spin" /></div>
            ) : (
              polls.map((poll) => {
                ...
                return (
                  <motion.div key={poll._id} layout initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="card p-6">
                    <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
                      <div className="flex-1 space-y-2">
                        <div className="flex gap-2 items-center">
                          <span className={statusBadge(poll.status)}>{poll.status}</span>
                          <span className="text-[10px] text-slate-400 font-bold">{poll.location}</span>
                        </div>
                        <h3 className="text-lg font-black text-slate-900">{poll.question}</h3>
                        <p className="text-sm text-slate-500 line-clamp-1">{poll.description}</p>
                        <div className="flex flex-wrap gap-4 text-xs text-slate-400 font-bold pt-1">
                      ...
                      <div className="flex gap-2 shrink-0">
                        <motion.button
                          ...
                          disabled={poll.status === "open" || actionLoading[poll._id + "_open"]}
                          className="flex items-center gap-1.5 px-5 py-2.5 bg-green-500 text-white rounded-xl text-xs font-black hover:bg-green-600 transition-colors disabled:opacity-40 disabled:cursor-not-allowed shadow-lg shadow-green-500/20"
                        >
                        ...
                        <motion.button
                          ...
                          disabled={poll.status === "closed" || actionLoading[poll._id + "_close"]}
                          className="flex items-center gap-1.5 px-5 py-2.5 bg-red-500 text-white rounded-xl text-xs font-black hover:bg-red-600 transition-colors disabled:opacity-40 disabled:cursor-not-allowed shadow-lg shadow-red-500/20"
                        >
